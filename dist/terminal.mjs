...
// Make these globals for HTML access
window.startingFolder = startingFolder;
...
document.querySelectorAll('.command-contents').forEach(button => {
    button.addEventListener('click', () => {
        ...
        switch(command) {
            case 'rm':
                window.displayCommandOptions(window.currentFolder, true, 'rm'); // show files
                break;
            case 'rmdir':
                window.displayCommandOptions(window.currentFolder, false, 'rmdir'); // show folders
                break;
            case 'cd':
                window.displayCommandOptions(window.currentFolder, false, 'cd'); // show folders to navigate
                break;
            default:
                window.processCommand(command); // normal commands like touch/mkdir
        }
    });
});
...
// Expose processCommand to window
...
function displayCommandOptions(folder, isForFiles = true, commandType = '') {
    ...
    items.forEach(item => {
        ...
        box.addEventListener('mouseout', () => box.style.backgroundColor = 'transparent');"use strict";
...
function processCommand(command) {
    ...
    document.getElementById("directory").innerHTML = currentFolder.name;
        ...
        box.addEventListener('click', () => {
            ...
            switch(commandType) {
                case 'rm':
                    window.processCommand(`rm|${item.name}`);
                    break;
                case 'rmdir':
                    window.processCommand(`rmdir|${item.name}`);
                    break;
                case 'cd':
                    window.processCommand(`cd|${item.name}`);
                    break;
                case 'touch':
                    window.processCommand(`touch|${item.name}`);
                    break;
                default:
...
// Expose displayCommandOptions to window
window.displayCommandOptions = displayCommandOptions;